...
public class LgDispatcherServlet extends HttpServlet {
    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                List<String> userWriteList = new ArrayList<String>(userWriteSet);
                // 把method所有信息及url封装为一个Handler
                Handler handler = new Handler(entry.getValue(),method, Pattern.compile(url), userWriteList);
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        for(Map.Entry<String,String[]> param: parameterMap.entrySet()) {
            if(param.getKey().equals("username")){
                if(Arrays.stream(param.getValue()).filter(t->handler.getUserWriteList().contains(t)).count() == 0){
    ...
    private String[] validateUser(Handler handler){
